...
public class Sach_dao {
    ...
    public static ArrayList<Sach_pojo> layDanhSachSach() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                hd.setTinhTrangSach(rs.getString("tinhTrangSach"));
                dsS.add(hd);
            }
    ...
    public static boolean themSach(Sach_pojo s) {
        boolean kq = false;
        String sql = String.format("Insert into Sach (maSach,tenSach,donGia,soLuongCon,maTG,maTL,tinhTrangSach) values ('%s','%s','%s','%s','%s','%s',N'%s')", s.getMaS(), s.getTenS(), s.getDonGia(), s.getSlCon(), s.getMaTG(), s.getMaTL(), s.getTinhTrangSach());
        SQLServerDataProvider ds = new SQLServerDataProvider();
        ds.open();
    ...
    public static Sach_pojo laySach(int maS) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                s.setTinhTrangSach(rs.getString("tinhTrangSach"));
            }
            ds.close();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static boolean capNhatSach(Sach_pojo s) {
        boolean kq = false;
        String sql = String.format("update SACH set tenSach=N'%s', donGia = '%s', soLuongCon = '%s', maTG = '%s', maTL='%s',tinhTrangSach=N'%s' where maSach='%s'", s.getTenS(), s.getDonGia(), s.getSlCon(), s.getMaTG(), s.getMaTL(), s.getTinhTrangSach(), s.getMaS());
        SQLServerDataProvider ds = new SQLServerDataProvider();
        ds.open();
        int n = ds.executeUpdate(sql);
        if (n == 1) {
    ...
    public static ArrayList<Sach_pojo> layDanhSachSachTrenKe() {
        ArrayList<Sach_pojo> dsS = new ArrayList<>();
        try {
            String sql = "Select * from SACH where tinhTrangSach = N'Đã lên kệ'";
            SQLServerDataProvider ds = new SQLServerDataProvider();
            ds.open();
            ResultSet rs = ds.executeQuery(sql);
            while (rs.next()) {
