...
public class KhachHang_dao {
    ...
    public static ArrayList<KhachHang_pojo> layDanhSachKhachHang() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                hd.setGiamGia(rs.getString("giamGia"));
    ...
    public static boolean themKhachHang(KhachHang_pojo kh) {
        boolean kq = false;
        String sql = String.format("Insert into KHACHHANG(maKH,tenKH,SDT,email) Values ('%s',N'%s','%s','%s')", kh.getMaKH(), kh.getTenKH(), kh.getSdtKH(), kh.getEmail());
    ...
    public static KhachHang_pojo layKhachHang(String maKH) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                kh.setGiamGia(rs.getString("giamGia"));
                kh.setLoaiThe(rs.getString("loaiThe"));
    ...
    public static boolean capNhatKhachHang(KhachHang_pojo kh) {
        ...
        String sql = String.format("Update KHACHHANG set tenKH ='%s' , SDT = '%s',"
                + "email='%s' Where maKH = '%S'", kh.getTenKH(), kh.getSdtKH(), kh.getEmail(), kh.getMaKH());
        SQLServerDataProvider provider = new SQLServerDataProvider();
        provider.open();
        int n = provider.executeUpdate(sql);
        if (n == 1) {
