...
public class HoaDon_dao {
    ...
    public static double layGiamGiaKhachHangTheoHoaDon(String makh, String mahd) {
        double giam = 0;
        try {
            String sql = String.format("select giamGia from HOADON,KHACHHANG where maHD = '%s' and hoadon.maKH='%s' and HOADON.maKH = KHACHHANG.maKH", mahd, makh);
            SQLServerDataProvider ds = new SQLServerDataProvider();
            ds.open();
            ResultSet rs = ds.executeQuery(sql);
            while (rs.next()) {
                giam = rs.getDouble("giamGia");
            }
            ds.close();
        } catch (SQLException ex) {
            Logger.getLogger(HoaDon_dao.class.getName()).log(Level.SEVERE, null, ex);
